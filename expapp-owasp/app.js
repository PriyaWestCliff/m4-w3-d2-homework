...
// Preventing DOS Attacks - Body Parser (Body size limit)
...
app.use(expSession({
  ...
  cookie: {
    httpOnly: true,
    secure: false,            // local dev (HTTP). Use true only with HTTPS
    maxAge: 1 * 60 * 1000     // 10 minutes (1 * 60 * 1000 ms) - Session expires after 10 minutes of inactivity
  }
}));
...
app.post("/register", async (req, res) => {
  ...
  try {
    await User.register(
      new User({ username, email, phone }),
      password
    );

    // After registered → go to login
